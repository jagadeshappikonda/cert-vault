...
app.get("/api/:employeeId/certificates/search/:certificateId", async (request, response)=>{
    try{
        const employeeId = request.params.employeeId;
        const certificateId = request.params.certificateId;
        const query = `select * from certificates where certificateId = ? and employeeId = ?`;
        const queryParameters = [certificateId, employeeId];
        const certificateDetails = await dbConnection.get(query, queryParameters);
        if (certificateDetails) {
            response.status(200).send(certificateDetails);
        } else {
            response.status(404).send(`Certificate with id ${certificateId} not found for employee ${employeeId}`);
        }
    }
    catch(error){
        console.error("Error updating certificate data:", error);
